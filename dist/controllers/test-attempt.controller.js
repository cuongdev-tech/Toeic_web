...
exports.TestAttemptController = {
    startTest: async (req, res) => {
        try {
            ...
            const test = await prisma_1.prisma.test.findUnique({ where: { id: String(testId), status: 'PUBLISHED' } });
            if (!test) {
                res.status(404).json({ success: false, message: 'Không tìm thấy đề thi' });
                return;
            }
            const activeAttempts = await prisma_1.prisma.testAttempt.findMany({
                where: { userId, status: client_1.AttemptStatus.IN_PROGRESS },
            ...
            if (expiredAttempts.length) {
                await prisma_1.prisma.testAttempt.updateMany({
                    where: { id: { in: expiredAttempts.map((item) => item.id) } },
                    data: { status: client_1.AttemptStatus.ABANDONED },
                });
            }
    ...
    syncAttempt: async (req, res) => {
        try {
            const attemptId = req.params.attemptId;
            const { timeRemaining, cheatWarningCount, answers } = req.body;
            const userId = req.user?.id;
            const attempt = await prisma_1.prisma.testAttempt.findFirst({ where: { id: attemptId, userId } });
            if (!attempt) {
            ...
            if (attempt.status !== client_1.AttemptStatus.IN_PROGRESS) {
                res.status(409).json({ success: false, message: 'Lượt làm bài không còn hoạt động' });
                return;
            }
            ...
            const ops = [];
            ops.push(prisma_1.prisma.testAttempt.update({
                where: { id: attemptId },
                data: {
            ...
            if (Array.isArray(answers)) {
                answers.forEach((ans) => {
                    ops.push(prisma_1.prisma.attemptAnswer.upsert({
    ...
    submitTest: async (req, res) => {
        try {
            ...
            const attempt = await prisma_1.prisma.testAttempt.findUnique({
                where: { id: attemptId }
            });
            if (!attempt) {
                res.status(404).json({ success: false, message: 'Không tìm thấy lượt làm bài' });
            ...
            if (attempt.userId !== userId) {
                res.status(403).json({ success: false, message: 'Bạn không có quyền nộp lượt làm bài này' });
                return;
            }
            ...
            const correctQuestions = await prisma_1.prisma.testQuestion.findMany({
                where: { testId: attempt.testId },
                include: { question: true }
            });
            const submittedAnswers = new Map(Array.isArray(answers) ? answers.map((answer) => [String(answer.questionId), answer.selectedOption || null]) : []);
            ...
            const answerRows = correctQuestions.map(({ question }) => {
                ...
                if (!partAnalytics[part])
                    partAnalytics[part] = { correct: 0, total: 0 };
            ...
            const { listeningScore, readingScore, totalScore } = (0, scoring_1.calculateSectionScores)(listeningCorrect, listeningTotal, readingCorrect, readingTotal);
            await prisma_1.prisma.$transaction([
                ...answerRows.map((answer) => prisma_1.prisma.attemptAnswer.upsert({
                ...
                prisma_1.prisma.testAttempt.update({
                    where: { id: attemptId },
                    data: {
    ...
    getAttemptReview: async (req, res, next) => {
        try {
            const attemptId = req.params.attemptId;
            const userPayload = req.user;
            const userId = userPayload?.id || userPayload?.userId;
            ...
            const attemptAnswers = await prisma_1.prisma.attemptAnswer.findMany({
    ...
    getMyHistory: async (req, res) => {
        try {
            ...
            if (minScore || maxScore) {
                where.totalScore = {};
                if (minScore)
                    where.totalScore.gte = Number(minScore);
                if (maxScore)
                    where.totalScore.lte = Number(maxScore);
            }
            if (from || to) {
                where.startedAt = {};
                if (from)
                    where.startedAt.gte = new Date(String(from));
                if (to) {
            ...
            const staleAttempts = attempts.filter((attempt) => attempt.status === client_1.AttemptStatus.IN_PROGRESS && attempt.test?.duration && Date.now() - attempt.startedAt.getTime() >= attempt.test.duration * 60 * 1000);
            if (staleAttempts.length) {
                await prisma_1.prisma.testAttempt.updateMany({ where: { id: { in: staleAttempts.map((attempt) => attempt.id) } }, data: { status: client_1.AttemptStatus.ABANDONED } });
                staleAttempts.forEach((attempt) => { attempt.status = client_1.AttemptStatus.ABANDONED; });
            }
